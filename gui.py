    ...
    def start_agent(self):
        ...
        def run_agent():
            try:
                ...
                self.message_queue.put(("start", cycles))
                ...
                for cycle in range(1, cycles + 1):
                    ...
                    try:
                        query, category, query_type = agent.generate_search_query()
                        self.message_queue.put(("query", f"Cycle {cycle}: {query}"))
                        success, url, execution_time = agent.execute_search(query)
                        
                        if success:
                            self.message_queue.put(("success", f"[SUCCESS] Search completed in {execution_time:.2f}s"))
                        else:
                            self.message_queue.put(("error", "[FAIL] Search failed"))
                        
                        progress = (cycle / cycles) * 100
                        self.message_queue.put(("progress", progress))
                        
                        # Random delay (simplified for GUI)
                        if cycle < cycles and self.is_running:
                            import random
                        ...
                        self.message_queue.put(("error", f"Cycle {cycle} failed: {e}"))
